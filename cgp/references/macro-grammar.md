...
The grammar every CGP macro parses, the code each one expands to, and how to read the compiler errors that result. This is the reference for writing CGP that parses, and for decoding what goes wrong when it does not.
...
- **`#[extend(Trait, Trait<P>, …)]`** takes the same simple-path list. It adds each as a *supertrait* of the generated trait (and, in `#[cgp_fn]`, also as an impl bound). Host: `#[cgp_fn]`, `#[cgp_component]`. Not `#[cgp_impl]`, because a provider impl has no trait to attach supertraits to.
